...
public sealed class UpdateCommandHandlerTests : IDisposable
{
    ...
    {
        // Arrange.
        var handler = new UpdateCommandHandler(_context, _mockValidator.Object, _mapper);

        var command = new UpdateCommand<UpdateCountryRequest, CountryResponse>
        {
            Request = new UpdateCountryRequest { Id = 1, Name = "Updated Country" }
        };

        _mockValidator.Setup(x => x.ValidateAllRules(EntityValidatorType.Update, It.IsAny<Country>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    public async Task Handle_ThrowsException_WhenValidationFails()
    {
        ...
        _mockValidator.Setup(x => x.ValidateAllRules(EntityValidatorType.Update, It.IsAny<Country>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception("Validation failed"));
