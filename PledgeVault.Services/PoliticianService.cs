...
public sealed class PoliticianService : IPoliticianService
{
    ...
    public async Task CheckIfPartyLeaderExists(Politician entity, CancellationToken cancellationToken)
    {
        ...
        if (await _context.Politicians.AnyAsync(x => x.Party.Id == entity.PartyId && x.IsPartyLeader, cancellationToken))
            throw new InvalidRequestException($"{nameof(Politician.IsPartyLeader)} already assigned for {nameof(Politician.PartyId)}: '{entity.PartyId}'");
    }
}